...
* **Middleware Alignment (March 2026):** `middleware.ts` was not updated when `SESSION_SIGNING_KEY` was introduced — it still used only `INTERNAL_SYNC_SECRET` for token verification. When both keys were set to different values, the middleware rejected every valid session token (HMAC mismatch), causing an infinite PIN login loop. Fixed: middleware now uses `SESSION_SIGNING_KEY || INTERNAL_SYNC_SECRET`, matching the precedence in `_auth.js` `signToken()` and `authorize()`.
