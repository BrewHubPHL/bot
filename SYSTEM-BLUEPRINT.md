...
- **Row Level Security**: Deny-all by default, scoped SELECT for authenticated staff.
  ...
  - **merch_products extended columns**: `long_description` (TEXT) for origin stories / tasting notes; `allowed_modifiers` (JSONB, default `[]`) for per-item modifier group keys (`milks`, `sweeteners`, `standard_syrups`, `specialty_addins`); `embedding` (VECTOR(1024)) for pgvector semantic search (Cohere `embed-english-v3.0`).
  - **pgvector Semantic Search**: `CREATE EXTENSION vector` enabled. `match_menu_items(query_embedding vector(1024), match_threshold, match_count)` RPC performs cosine similarity (`<=>`) search on `merch_products.embedding`. IVFFlat index (`idx_merch_products_embedding`) for ANN. SECURITY INVOKER — inherits caller's RLS (public sees active items only, staff sees all). Embedding provider: Cohere (was OpenAI prior to 2026-03-05).
  ...
  - **store_settings** (Schema-90): RLS enabled + forced, all grants revoked from `anon`/`authenticated`, explicit deny-all policies. Only `service_role` (Netlify functions) can read `shop_ip_address`.
...
### High-Value Parcel Pickup Code Generation
- **Server Action**: `generatePickupCode` in `src/app/(ops)/actions/parcel-actions.ts`.
- **Gate**: Only parcels with `estimated_value_tier` = `high_value` or `premium` and status `arrived`/`in_lobby`.
- **Auth**: CSRF header + HMAC-signed staff PIN session cookie (verified in-action, not dependent on middleware).
- **Code**: `crypto.randomInt(100000, 999999)` → HMAC-SHA256 hash stored on `parcels.pickup_code_hash`.
- **Audit**: `parcel_pickup_log` entry with `attempt_type: 'code_generated'` and `code_hash` (migration `20260305_pickup_code_generation`).
- **SMS**: Dispatched via `send-sms-email` Netlify function (service-to-service auth) → TCPA-compliant `_sms.js` gateway.
...
- **SecurityAlertToaster** (`src/components/manager/SecurityAlertToaster.tsx`): Subscribes to `postgres_changes` INSERT events on `manager_override_log` filtered to `action_type=rate_limit_triggered`. Displays Sonner toasts with redacted IP hash and source function. Gated behind a valid staff_pin session via `useOpsSessionOptional`.
- **ManagerOverrideLog** (`src/components/manager/ManagerOverrideLog.tsx`): Full audit table of `manager_override_log` entries rendered via Shadcn Data Table (`@tanstack/react-table`). Sortable columns, pagination, and dropdown actions (copy ID / details / target). Displays a red `Badge` component for `challenge_method = 'none_legacy'`. Data fetched from `get-override-log` Netlify function (manager-only). Mounted in the Overview tab of the Manager Dashboard.
...
### AI SDK Migration — Vercel AI SDK + `check_loyalty_points` Tool (`claude-chat.js`)
- **Breaking change:** Replaced raw `fetch()` calls to `api.anthropic.com` with Vercel AI SDK (`ai` + `@ai-sdk/anthropic`).
  - Uses `generateText()` with `maxSteps: 3` — the SDK manages the full tool-call loop automatically.
  - Tools defined via AI SDK `tool()` with Zod schemas instead of raw JSON Schema `input_schema`.
  - Tool `execute()` functions use closures over request-scoped context (`supabase`, `authedUser`, `clientIp`) instead of injecting `_authed_user`/`_client_ip` into tool input.
- **New Tool:** `check_loyalty_points` queries unified `customers` table for the authenticated user's `loyalty_points`.
  - Returns structured `{ type: 'loyalty_card', points, tier, ... }` data.
  - Frontend renders a Shadcn `Card` component (`LoyaltyPointsCard.tsx`) with tier badge, progress bar, and portal link.
  - Identity-bound: only returns data for the JWT-verified user, ignoring AI-provided email.
- **Allergen safety preserved:** All three safety layers (pre-LLM regex gate, system prompt, post-response scrubber) unchanged.
- **Response format:** JSON body now includes optional `loyaltyCard` and `orderConfirmation` fields alongside `reply` when their respective tools fire.
  - `orderConfirmation`: `{ order_id, customer_name, amount_cents }` — extracted from `place_order` tool results.
- **Frontend:** `EliseChat.tsx` updated to destructure `{ reply, loyaltyCard, orderConfirmation }` and render `LoyaltyPointsCard` inline in chat bubbles.
  - **Order toast:** A `useEffect` monitors the messages array for `orderConfirmation` payloads and fires a Sonner `toast.success('Order Confirmed! ☕')` with the short order number and formatted price. A `Set<string>` ref (`toastedOrdersRef`) ensures each order ID only triggers one toast, preventing duplicates on re-render.
...
- **Idempotency Table:** `processed_notion_syncs` (Schema 86) stores unique `sync_key` entries to guarantee at-most-once Notion writes.
  - **Status-Based Retry (Schema 88):** Added `status` column (`pending` → `completed` | `failed`), `notion_page_id`, and `updated_at`. Only `completed` rows block duplicates (via partial unique index). Failed or crashed syncs can be retried.
  - **Atomic RPCs:** `claim_notion_sync()` acquires a `pg_advisory_xact_lock` on the sync key hash, checks for existing rows, and returns `'claimed'`/`'duplicate'`/`'retry'`. `complete_notion_sync()` and `fail_notion_sync()` transition the row status after the Notion API call succeeds or fails.
- **DB Trigger Webhooks:**
...
### Drop Legacy Plaintext PIN (`20260305_schema89_drop_plaintext_pin`)
- **Migration:** Drops the plaintext `pin` column from `staff_directory`. All PIN verification now uses `verify_staff_pin` RPC (bcrypt via `pin_hash`) exclusively.
- **Safety Guard:** Migration aborts if any active staff still have a plaintext PIN without a corresponding `pin_hash`.
- **Code Cleanup:** Removed plaintext PIN fallback from `process-comp.js` (legacy `safeCompare` loop over all staff rows). Removed dead `pin: null` update from `pin-change.js`.
...
- **`record_agreement_signature` RPC:** Atomic Postgres function (`SECURITY DEFINER`, `search_path = public`). Uses `pg_advisory_xact_lock` to serialise concurrent signatures for the same staff member. In a single transaction: (a) inserts into `agreement_signatures`, (b) updates `staff_directory.contract_signed` + `onboarding_complete`, (c) bumps `token_version` + `version_updated_at` to atomically invalidate all existing sessions — forcing a fresh login that picks up the new `onboarding_complete = true` status. **Schema-91 hardening (Ticket C-1):** after the UPDATE, uses `GET DIAGNOSTICS ROW_COUNT` to verify exactly one row was touched; if zero (staff deactivated/missing), raises `ERRCODE P0002` which rolls back the entire transaction including the INSERT — preventing partial audit-trail-without-contract states. Returns `{ success, signature_id, signed_at }`. Migration: `20260305_schema91_atomic_agreement_rpc.sql`.
...
- **`record-agreement-signature.js`:** POST endpoint (`{ pin, staff_id, version_tag? }`) — verifies PIN via `verify_staff_pin` RPC, validates `staff_id` matches authenticated session. **(Ticket H-1 — Canonical Agreement Hashing):** fetches the employee's `full_name` and `hourly_rate` from `staff_directory`, then regenerates the agreement entirely server-side via `generateStaffAgreement()` + `getCanonicalAgreementText()` — the client never supplies `agreement_text`. SHA-256 hashes this server-generated canonical string for the audit row. **Version pinning:** the `version_tag` stored in the DB is always the server-side constant `CURRENT_VERSION` (`'2027-Q1'`); client-supplied version_tag is logged for drift detection only. Strictly validates the RPC `success` flag and `signature_id` presence — returns 409 if staff record is inactive (P0002 rollback) and 500 if the response payload is malformed. Sends manager notification email via Resend.
...
- **Backend:** All order-creation endpoints (`cafe-checkout.js`, `ai-order.js`, `create-order.js`, `create-checkout.js`, `process-merch-payment.js`, `claude-chat.js`) use `calculateTaxInclusive()` to derive accounting fields. All three columns (`subtotal_cents`, `tax_amount_cents`, `total_amount_cents`) are written to the `orders` table, where `total_amount_cents` equals the exact menu price.
- **Comp orders:** Tax-exempt — `subtotal_cents = total_amount_cents`, `tax_amount_cents = 0`.
